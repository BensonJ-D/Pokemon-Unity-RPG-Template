...
namespace Menus.Party
{
    ...
    {
        [Separator("Party Menu")] [SerializeField]
        private List<PartyMenuItem> menuItems;
        ...
        public IEnumerator OpenWindow(PokemonParty pokemon, bool isBattle = false,
        ...
        private IEnumerator DefaultOnConfirmCallback(Pokemon choice) {
            if (State == PartyMenuState.Switch) {
                ...
                var switchFromAnimateOut = new Task(fromSlot.ShiftOut());
                var switchToAnimateOut = new Task(toSlot.ShiftOut());
        ...
        public IEnumerator TypeMessage(string message) {
            return messageTextBox.TypeMessage(message);
        }
    }
}