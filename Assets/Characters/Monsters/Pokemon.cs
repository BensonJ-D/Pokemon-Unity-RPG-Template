...
namespace Characters.Monsters
{
    ...
    public class Pokemon
    {
        ...
        private int MaxHp(int level) {
            return Mathf.FloorToInt((2 * Base.MaxHp * level) / 100f) + 10 + level;
        }

        private int Attack(int level) {
            return Mathf.FloorToInt((2 * Base.Attack * level) / 100f) + 5;
        }

        private int Defence(int level) {
            return Mathf.FloorToInt((2 * Base.Defence * level) / 100f) + 5;
        }

        private int SpAttack(int level) {
            return Mathf.FloorToInt((2 * Base.SpAttack * level) / 100f) + 5;
        }

        private int SpDefence(int level) {
            return Mathf.FloorToInt((2 * Base.SpDefence * level) / 100f) + 5;
        }

        private int Speed(int level) {
            return Mathf.FloorToInt((2 * Base.Speed * level) / 100f) + 5;
        }
        ...
        public int SpAttack() {
            return SpAttack(Level);
        }

        public int SpDefence() {
            return SpDefence(Level);
        }
        ...
        private int GetBoostedStat(Stat statType, int statValue) {
            return (int) (StatBoosts[statType] < 0
