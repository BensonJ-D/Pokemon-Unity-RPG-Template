...
namespace Characters.UI
{
    public class HealthBar : MonoBehaviour
    {
        ...
        public IEnumerator UpdateHealth(int healthDelta, uint updatePercentageSpeed = 100)
        {
            var targetHp = Mathf.Clamp(healthDelta + Hp, 0, MaxHp);
            var deltaDirection = Mathf.Abs(healthDelta) / healthDelta;
            var healthStepSize = MaxHp / 100f;
            
            var updateMultiplier = updatePercentageSpeed / 100f;
            while(Mathf.Abs(targetHp - Hp) > 0)
            {
                var newHp = Mathf.Clamp(Hp + healthStepSize * deltaDirection, 0, MaxHp);       
